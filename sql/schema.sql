...
CREATE TABLE IF NOT EXISTS `transactions` (
  `id`         BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `user_id`    INT UNSIGNED NOT NULL,
  `type`       ENUM('faucet','shortlink','ptc','offerwall','referral','withdraw','admin','bonus','daily_bonus','achievement','bingo_buy','bingo_win','instant','instant_ticket','gift','quiz') NOT NULL,
  `coins`      DECIMAL(18,4) NOT NULL DEFAULT 0,
...
INSERT IGNORE INTO `homepage_blocks` (`k`,`title`,`content`,`visible`,`sort_order`) VALUES
 ('hero',
  ...
  'Réclamez des Coins toutes les 3 heures, complétez des shortlinks, faites grimper votre XP et invitez vos amis pour gagner 10% sur tous leurs gains.',
  1, 1),
...
-- On evite ainsi information_schema, dont l'acces est refuse a
-- l'utilisateur de la base sur un hebergement mutualise (#1044).
...
INSERT IGNORE INTO `achievements`
  ...
  ('faucet_10',     'faucet_claims',      10,    'bronze',   'Goutte à goutte',    'Réclamez 10 fois le faucet',           '💧', 20,   10,  10),
  ('faucet_100',    'faucet_claims',      100,   'silver',   'Robinet ouvert',     'Réclamez 100 fois le faucet',          '🚰', 100,  50,  11),
  ('faucet_1000',   'faucet_claims',      1000,  'gold',     'Cascade',            'Réclamez 1000 fois le faucet',         '🌊', 1000, 250, 12),
  ('shortlink_10',  'shortlinks_done',    10,    'bronze',   'Premiers liens',     'Complétez 10 shortlinks',              '🔗', 25,   10,  20),
  ('shortlink_100', 'shortlinks_done',    100,   'silver',   'Chaîne solide',      'Complétez 100 shortlinks',             '⛓️', 120,  60,  21),
  ('shortlink_500', 'shortlinks_done',    500,   'gold',     'Maître des liens',   'Complétez 500 shortlinks',             '🏅', 600,  200, 22),
...
VALUES (
 ...
 'Vous voulez tirer le meilleur parti de votre temps sur Wintaskly ? Découvrez 7 stratégies concrètes et éprouvées pour augmenter vos gains sans y passer vos journées.',
 '💡',
 'Équipe Wintaskly',
...
VALUES (
 ...
 (SELECT id FROM blog_categories WHERE slug='guides'),
 'Sécurité en ligne : comment protéger votre compte et éviter les arnaques',
 'Protéger votre compte et vos gains est essentiel. Découvre les bonnes pratiques de sécurité, les signes d''une arnaque, et les réflexes à adopter pour naviguer sereinement.',
 '🔒',
 'Équipe Wintaskly',
 'Sécurité en ligne : protéger son compte et éviter les arnaques',
 'Guide de sécurité en ligne : mots de passe forts, authentification à deux facteurs, détection des arnaques et phishing. Protège votre compte efficacement.',
 'published', 6, UTC_TIMESTAMP(),
 '<p>Sur internet, votre sécurité dépend largement de vos habitudes. Que vous utilisez une plateforme de micro-gains, une messagerie ou un service bancaire, les principes de protection restent les mêmes. Ce guide vous donne les réflexes essentiels pour protéger votre compte, vos gains et vos données personnelles.</p>
...
-- ---------------------------------------------------------------------
ALTER TABLE `ad_zones` ADD COLUMN IF NOT EXISTS `fail_streak` smallint(5) unsigned NOT NULL DEFAULT 0;

ALTER TABLE `ad_zones` ADD COLUMN IF NOT EXISTS `last_check_at` datetime NULL;

ALTER TABLE `ad_zones` ADD COLUMN IF NOT EXISTS `last_http_code` smallint(5) unsigned NOT NULL DEFAULT 0;

ALTER TABLE `blog_posts` ADD COLUMN IF NOT EXISTS `cover_image` varchar(190) NULL;

ALTER TABLE `blog_posts` ADD COLUMN IF NOT EXISTS `cover_prompt` text NULL;

ALTER TABLE `offerwalls` ADD COLUMN IF NOT EXISTS `fail_streak` smallint(5) unsigned NOT NULL DEFAULT 0;

ALTER TABLE `offerwalls` ADD COLUMN IF NOT EXISTS `last_check_at` datetime NULL;

ALTER TABLE `offerwalls` ADD COLUMN IF NOT EXISTS `last_http_code` smallint(5) unsigned NOT NULL DEFAULT 0;

ALTER TABLE `ptc_ads` ADD COLUMN IF NOT EXISTS `fail_streak` smallint(5) unsigned NOT NULL DEFAULT 0;

ALTER TABLE `ptc_ads` ADD COLUMN IF NOT EXISTS `last_check_at` datetime NULL;

ALTER TABLE `ptc_ads` ADD COLUMN IF NOT EXISTS `last_http_code` smallint(5) unsigned NOT NULL DEFAULT 0;

ALTER TABLE `shortlinks` ADD COLUMN IF NOT EXISTS `fail_streak` smallint(5) unsigned NOT NULL DEFAULT 0;

ALTER TABLE `shortlinks` ADD COLUMN IF NOT EXISTS `last_check_at` datetime NULL;

ALTER TABLE `shortlinks` ADD COLUMN IF NOT EXISTS `last_http_code` smallint(5) unsigned NOT NULL DEFAULT 0;

ALTER TABLE `users` ADD COLUMN IF NOT EXISTS `twofa_email_enabled` tinyint(1) NOT NULL DEFAULT 0;

ALTER TABLE `users` ADD COLUMN IF NOT EXISTS `twofa_phone` varchar(32) NULL;

ALTER TABLE `users` ADD COLUMN IF NOT EXISTS `twofa_phone_verified_at` datetime NULL;

ALTER TABLE `users` ADD COLUMN IF NOT EXISTS `twofa_preferred` enum('totp','email','sms') NULL;

ALTER TABLE `users` ADD COLUMN IF NOT EXISTS `twofa_sms_enabled` tinyint(1) NOT NULL DEFAULT 0;
